...
import { defaultRules, AnomalyRule } from './rules.js';
...
function checkThreshold(
  value: number,
  rule: AnomalyRule
): { triggered: boolean; severity: 'warning' | 'critical' | null } {
...
function addAlert(alert: AnomalyAlert): void {
...
export async function checkSystemAnomalies(): Promise<AnomalyAlert[]> {
  logger.debug('Checking system anomalies');

  const currentAlerts: AnomalyAlert[] = [];
  const now = Date.now();
...
export function getAlertHistory(limit?: number): AnomalyAlert[] {
