...
import { ThermalInfo } from '../types/system.types.js';
...
export async function getThermalInfo(): Promise<ThermalInfo> {
  ...
  if (temps.main !== null && temps.main > 0) {
    return {
      main: temps.main,
      cores: temps.cores.filter((t) => t !== null) as number[],
...
async function getThermalFromNative(): Promise<ThermalInfo | null> {
  try {
    // Use powermetrics for Apple Silicon (requires sudo for full data)
    // Alternative: use ioreg for basic thermal data
    const { stdout } = await execAsync(
      'ioreg -r -c AppleSmartBattery 2>/dev/null | grep -i temperature || echo ""',
      { timeout: 5000 }
    );
